...
import br.gov.sp.sme.salaleitura.feature.security.OperatorGate
...
fun SalaLeituraApp(context: Context = LocalContext.current) {
    SalaLeituraTheme {
        OperatorGate {
            ...
            LaunchedEffect(Unit) {
                hasSchool = withContext(Dispatchers.IO) { AppDatabase.get(context).schoolDao().get() != null }
            }
            if (hasSchool == null) CircularProgressIndicator()
            else AppNav(hasSchool = hasSchool == true)
        }
    }
}
